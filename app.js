...
function selectStyle(style) {
  ...
  $('modeInstruction').textContent = `${modeInstructions[mode]}${competitive ? '\nScores appear at the end of each round.' : ''}`;
...
if ('serviceWorker' in navigator) window.addEventListener('load', async () => {
  try {
    const registration = await navigator.serviceWorker.register('./service-worker.js?v=52-pricing', {updateViaCache: 'none'});
