...
const monetization = fs.readFileSync(new URL('../monetization.js', import.meta.url), 'utf8');
const checkout = fs.readFileSync(new URL('../api/create-checkout-session.js', import.meta.url), 'utf8');
...
assert.match(sw, /al-majlis-v52-pricing/);
...
assert.ok(html.includes('./monetization.css?v=5') && html.includes('./monetization.js?v=6'), 'source upload loads cache-busted premium access controls directly');
assert.ok(app.includes("service-worker.js?v=52-pricing"), 'service-worker registration uses the current pricing release URL');
assert.equal((monetization.match(/price: '\$1\.99'/g) || []).length, 6, 'all six individual premium modes display the $1.99 price');
assert.match(monetization, /price: '\$7\.99'/, 'premium all access remains $7.99');
assert.equal((checkout.match(/amount:199/g) || []).length, 6, 'Stripe charges $1.99 for every individual premium mode');
assert.match(checkout, /bundle:\{[^\n]*amount:799/, 'Stripe keeps premium all access at $7.99');
assert.match(policies, /purchases for \$1\.99 each/);
...
assert.match(policies, /Privacy policy/);
assert.match(policies, /Terms of use/);
assert.match(policies, /m\.alqaddi@outlook\.com/);
...
assert.ok(!app.includes('usesBrowserInstallMenu'), 'Android Edge can use the native install prompt when available');
assert.match(app, /\$\('install'\)\.onclick = \(\) => installPrompt\s*\? runNativeInstall\(\)/, 'Install App opens the native browser prompt before falling back to instructions');
...
assert.match(css, /--bg:#e7e5e2;[\s\S]*--marble:url\('\.\/assets\/marble-light\.webp'\)/, 'light mode uses the preserved original palette and marble');
assert.doesNotMatch(css, /\[data-theme="light"\] body,[\s\S]*background-color:#aeb6b3/, 'the rejected grey-wash override stays removed');
